...
func libraryFilename() (string, error) {
	switch runtime.GOOS {
	case "darwin":
		return "libpgmicro_sdk_kit.dylib", nil
	case "linux":
		return "libpgmicro_sdk_kit.so", nil
	case "windows":
		return "pgmicro_sdk_kit.dll", nil
	default:
...
func embeddedLibraryTryCreate() (path string, err error) {
	cacheRoot := os.Getenv("PGMICRO_GO_CACHE_DIR")
	if cacheRoot == "" {
	...
	hashPath := hash[0:8]
	cacheDir := filepath.Join(cacheRoot, "pgmicro-go", hashPath)
